		...
		bindEvents: function() {
		  ...
		  try {
		  	...
		  	<c:if test="${col.ui == 'ueditor'}">
			      ...
			      if (${pageId}.data && ${pageId}.data['${col.id}']) {
			      	this.setContent(${pageId}.data['${col.id}']);
			      }
			  	});
		  	</c:if>
		  } catch (error) {
						...
						<table id="${pageId}_formTable" width="99%">
							<tr class="trEdit">
								...
								<c:forEach var="col" items="${validColumns}" varStatus="status">
									...
									<td width="10%" nowrap align="right">${not empty col.label?col.label:col.field }:
									</td>
									<td nowrap width="${(col.type=='MEMO'?100:100/columnCount)-11 }%" colspan="${col.type=='MEMO'?(columnCount*3-2):1 }"><c:choose>
											...
											<c:when
												...
												<select id="${pageId }_${col.id }" name="${col.id }"
													class="easyui-combobox" style="width: ${showAsDialog == '1'?'100':'30'}%;"
								                    ...
								                    <c:if test="${not empty col.onChange }">
								                    ,onChange : function(newValue, oldValue) { if(${pageId}.isValueInited) ${pageId}.${col.onChange}(${pageId}, newValue, '${col.id}_change',0);}
								                    </c:if>
													,readonly:${!col.editable },panelMinHeight:50, panelMaxHeight:100"
													ui="easyui-combobox" canInput="${view.dictMap[col.dictionary].canInput}">
											...
											<c:when test="${col.ui == 'easyui-combotree' && not empty col.valueRef}">
												<select id="${pageId }_${col.id }" name="${col.id }"
													class="easyui-combotree"  style="width: ${showAsDialog == '1'?'100':'30'}%;"
													ui="easyui-combotree"
													data-options="readonly : ${!col.editable},url:'getTreeData?fwCoord.function=${col.valueRef.function }&fwCoord.view=${col.valueRef.view }&fwCoord.project=${view.coord.project }&fwCoord.lang=${view.coord.lang }&fwCoord.ui=${view.coord.ui }',
											...
											<c:when
												...
												<select id="${pageId }_${col.id }" class="easyui-combogrid"
													...
													data-options="
														...
														url:'getRefData?fwParam.distinct=${col.valueRef.distinct }&fwParam.queryColumns=${col.valueRef.value },${col.valueRef.label },${col.valueRef.otherRefs }&fwCoord.function=${col.valueRef.function }&fwCoord.view=${col.valueRef.view }&fwCoord.project=${view.coord.project }&fwCoord.lang=${view.coord.lang }&fwCoord.ui=${view.coord.ui }',
											            columns:[[
											            ...
											            onLoadSuccess : function(data) {
									                      if (${pageId }.data) {
									                        ...
									                        $.each(data.rows, function(key, rowData) {
									                                  if (rowData['${ col.valueRef.value}'] == ${pageId }.data['${ col.id}']) {
									                                    exists = true;
									                                  }
									                                });
									                        if (!exists) {
									                          var so = {};
									                          so['${ col.valueRef.value}'] = ${pageId }.data['${col.id}'];
											...
											<c:when test="${col.ui == 'easyui-datetimebox' }">
												<input type="text" id="${pageId }_${col.id }" placeholder="${col.prompt }"
													...
													data-options="validateOnCreate:false,validateOnBlur:true,novalidate:true,required:${col.required },
													validType:['datetime[\'${col.pattern }\']','fwRemote[${pageId },\'${col.id }\',\'\']'],
													 readonly:${!col.editable }"
													ui="${col.ui }" pattern="${col.pattern }"/>
											</c:when>
											<c:when test="${col.ui == 'easyui-datebox' }">
												<input type="text" id="${pageId }_${col.id }" placeholder="${col.prompt }"
													...
													data-options="validateOnCreate:false,validateOnBlur:true,novalidate:true,required:${col.required },
													validType:['date[\'${col.pattern }\']','fwRemote[${pageId },\'${col.id }\',\'\']'],
													readonly:${!col.editable }"
													ui="${col.ui }" pattern="${col.pattern }"/>
											</c:when>
											<c:when test="${col.ui == 'easyui-timespinner' }">
												<input type="text" id="${pageId }_${col.id }" placeholder="${col.prompt }"
													name="${col.id }" style="width: ${showAsDialog == '1'?'100':'30'}%;"
													validType="fwRemote[${pageId },'${col.id }','']"
													class="easyui-timespinner"
													data-options="validateOnCreate:false,validateOnBlur:true,novalidate:true,required:${col.required },
													readonly:${!col.editable },showSeconds:false"
													ui="${col.ui }" />
											</c:when>
											<c:when test="${col.ui == 'easyui-numberbox' }">
												<input type="text" id="${pageId }_${col.id }" placeholder="${col.prompt }"
													name="${col.id }" style="width: ${showAsDialog == '1'?'100':'30'}%;"
											...
											<c:when test="${col.ui == 'ueditor' }">
												...
												<script id="${pageId }_${col.id }" name="${col.id }" ui="${col.ui }" type="text/plain" style="width:${col.width}px;height:${col.height}px;"></script>
											</c:when>
											<c:otherwise>
												<input type="${not empty col.ui?col.ui:'text' }" id="${pageId }_${col.id }" placeholder="${col.prompt }"
													...
													readonly:${!col.editable }, prompt:'${col.prompt }'<c:if test="${col.type =='MEMO'}">,multiline:true,height:${(not empty col.height)?col.height:'50' } </c:if>
													"
													ui="easyui-textbox" />
											</c:otherwise>
