	...
	, hidden:${!instance.visible}
		<c:choose>
			...
			<c:when test="${not empty instance.dictionary}">
				, formatter: function(val,data,index){return got.xssFilter(${pageId}.dictMap['${instance.dictionary}'][data['${instance.id}']]);}
			</c:when>
		...
		$(${pageId}.getGrid()).datagrid(datagridOptions);
		$(${pageId}.getGrid2()).datagrid(datagridOptions);
...
<jsp:include page="../grid_button_area.jsp" />
			...
			<table width="100%">
				<tr>
					...
					<td width="50%">
						<table id="${pageId}_datagrid2">
							<tbody id="${pageId}_dataArea2">
							</tbody>
						</table>
					</td>
				</tr>
