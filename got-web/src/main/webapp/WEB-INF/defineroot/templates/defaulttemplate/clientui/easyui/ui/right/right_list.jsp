					...
					got.ajax({
					  ...
					  success : function(returnData) {
					    ...
					    if (returnData && returnData["data"] && returnData["data"][0]) {
							var functionCheckStatus = $(document.getElementById('${pageId }_' + groupId +'_' + functionId)).attr('checked'); //"";
					    	...
					    	*/
					    	...
					    	for (var i = 0; i < actionList.length; ++i) {
					    		${pageId}.data[actionList[i]["PKEY"]] = {'HAVE_PRIVILEGE':actionList[i]['HAVE_PRIVILEGE']};
					    		if (i > 0 && i % 4 == 0) {
					    		...
					    		+"' name='"+actionList[i]["PKEY"]+"' type=checkbox value='1' "+(actionList[i]["HAVE_PRIVILEGE"]==1?"checked":checkedStr) 
							...
							$(document.getElementById('${pageId }_div_actions_' + functionId)).panel('close');
					    }
					    if (columnList != null && columnList.length > 0) {
					    	var columnHtml = "<table width=\"100%\"><tr>";
					    	for (var i = 0; i < columnList.length; ++i) {
					    		${pageId}.data[columnList[i]["PKEY"]] = {'HAVE_PRIVILEGE':columnList[i]['HAVE_PRIVILEGE']};
					    		if (i > 0 && i % 4 == 0) {
					    		...
					    		+"' name='"+columnList[i]["PKEY"]+"' type=checkbox value='1' "+(columnList[i]["HAVE_PRIVILEGE"]==1?"checked":checkedStr) 
					    	...
					    	$(document.getElementById('${pageId }_div_columns_' + functionId)).panel('close');
					    }
							/*
		...
		<c:forEach var="function" items="${group.children}" varStatus="funcStatus">
			<div id="${pageId }_div+${group.PKEY}+${function.PKEY}" class="easyui-panel" 
			        ...
			        style="width:100%; height:0px; padding:${function.type=='url'?'0':'10' }px;background:#fafafa;"
		                ...
		                <c:if test="${function.TYPE=='function' || function.TYPE=='url'}">
		                	...
		                	data-options="iconCls:'fwicon-datalevel',collapsible:true,collapsed:false">
			                	...
			                	<c:forEach var="dl" items="${view.dictMap['DATA_LEVEL'].itemList}" varStatus="dlStatus">
			                		<c:if test="${dl.value < 1 || (function.HAVE_CUST_COL == '1' && dl.value > 0 && dl.value < 9) || (function.HAVE_DEPT_COL == '1' && dl.value > 10 && dl.value < 19) || (function.HAVE_OWNER_COL == '1' && dl.value > 20 && dl.value < 29)}">
										<td nowrap><input id='${pageId }_${group.PKEY}_${function.PKEY}_datalevel_${dl.value}' name='${function.PKEY}' type=checkbox value='${dl.value }' ${(function['DATA_LEVEL_MAP'][dl.value]=='1')?'checked':'' } /><label for='${pageId }_${group.PKEY}_${function.PKEY}_datalevel_${dl.value}' >${dl.label}</label>
									</c:if>
									</td>
