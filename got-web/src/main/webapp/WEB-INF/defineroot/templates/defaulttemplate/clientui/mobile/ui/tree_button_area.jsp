...
<c:if test="${treeActions != null }">
<c:forEach var="act" items="${treeActions}" varStatus="status">
	<c:if test="${act.tag=='group' }">
		<c:if test="${act.id=='searchGrp' || act.id == 'treeSearchGrp' }">
			<input type="text" style="width:0px; display:none;"/>${act.children[0].label}: <input type="text" id="${pageId }_treeQueryValue" name="fwParam.queryValue" value="" onkeyUp="if (event.keyCode==13) {${pageId }.${ act.children[0].click}(${pageId }, null, '${act.children[0].id}', null, event);}" />
		</c:if>
	...
	<c:if test="${act.tag != 'group' }">
		<c:if test="${act.id=='searchTree' }">
			<input type="text" style="width:0px; display:none;"/>${act.label }: <input type="text" id="${pageId }_treeQueryValue" name="fwParam.queryValue" value="" onkeyUp="if (event.keyCode==13) {${pageId }.${act.click }(${pageId}, null, '${act.id }', null, event);}" />
		</c:if>
