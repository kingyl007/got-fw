...
var params = ["project=<%=request.getParameter("_project")%>"];

<%
	String[] paramKeys = new String[]{"_w","_h"};
	String[] toFlexKeys = new String[] {"width",""};
	for (int i = 0; i < paramKeys.length; ++i) {
		if (request.getParameter(paramKeys[i]) != null) {
			%>
			params.push(toFlexKeys[i] + "=" + <%=request.getParameter(paramKeys[i]) %>)
			<%
		}
	}
