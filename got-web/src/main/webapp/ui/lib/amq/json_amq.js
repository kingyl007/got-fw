  ...
  var clientId = null;
	...
	var messageHandler = function(data) {
		if (data != null && data.length > 0) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				// var id = msg.id;
				var topic = msg.destination;
				var invalidHandlers = [];
				var handlerIds = topicToIds[topic];
				if (handlerIds == null) {
				  if (logging) {
				    adapter.log('No handler found to match message with topic = ' + topic);
				  }
          continue;
				} else {
				  $.each(handlerIds, function(li, id) {
				  ...
				  for(var j = invalidHandlers.length-1; j >=0;--j) {
				    org.activemq.Amq.removeListener(handlerIds[invalidHandlers[j]], topic);
				  }
				}
			}
		}
	...
	return {
		...
		addListener : function(id, destination, handler, options) {
		  // already have topic register,just add to arr
		  if (topicToIds[destination]) {
		    var found = false;
		    $.each(topicToIds[destination], function(i, lid) {
		      if (lid == id) {
		        found = true;
		      }
		    });
		    if (!found) {
		      topicToIds[destination].push(id);
		    }
		  } else {
		    topicToIds[destination] = [id];
		...
		removeListener : function(id, destination) {
			...
			if (topicToIds[destination]) {
			  // if want to delete first element, check if not have any 
			  if (topicToIds[destination][0] == id) {
			    if (topicToIds[destination].length == 1) {
			      delete topicToIds[destination];
			      sendJmsMessage(destination, id, 'unlisten');
			    }
			  } else {
			    for(var i = topicToIds[destination].length - 1; i > 0; --i) {
			      if (topicToIds[destination][i] == id) {
			        topicToIds[destination].splice(i,1);
			      }
			    }
			  }
			}
