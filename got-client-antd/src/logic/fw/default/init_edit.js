...
export function logic(view, actionId, record, recordIndex, event) {
  ...
  this.view.props.getEditData({fwCoord:this.view.getFwCoord(), fwParam:{selectedData:{...this.view.props.selectedData}}, callback:(data)=> {
    const formData = {};
    Object.keys(data).map(key=>formData[key]={value:data[key]});
    this.view.setState({fwParam:{...this.view.state.fwParam, oldData:{...data}, newData:{...data}}, formData, manualValues:{}, refData:{}});
  }});
}
