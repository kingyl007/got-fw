...
export function logic(view, actionId, record, recordIndex, event) {
  ...
  const layoutCallback = (layout)=> {
    ...
    const clientNewData = {};
    console.info('this.record',this.record);
    clientNewData[PARENT_FIELD] = (this.record&&this.record.GOT_PID?this.record.GOT_PID:this.view.state.treeData[0][ID_FIELD]);
